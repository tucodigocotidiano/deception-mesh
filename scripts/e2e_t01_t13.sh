...
T13_HTTP_USER_AGENT="${T13_HTTP_USER_AGENT:-dm-e2e-http/1.0}"
...
cleanup() {
  ...
  if [ -n "${T13_AGENT_PID:-}" ]; then
    kill "$T13_AGENT_PID" 2>/dev/null || true
    wait "$T13_AGENT_PID" 2>/dev/null || true
  fi

  if [ -n "${T13_TMP_CFG:-}" ] && [ -f "${T13_TMP_CFG:-}" ]; then
    rm -f "$T13_TMP_CFG"
  fi

  if [ -n "${T13_LOG_FILE:-}" ] && [ -f "${T13_LOG_FILE:-}" ]; then
    rm -f "$T13_LOG_FILE"
  fi
}
trap cleanup EXIT
...
prepare_sensor_cfg_from_template() {
  ...
  local ssh_banner="$8"
  local http_listen_addr="$9"
  local https_listen_addr="${10}"
  local http_trap_paths_literal="${11}"
...
http_trap_paths = $http_trap_paths_literal
EOF
  fi
  ...
  sed -i \
    ...
    -e "s|^http_listen_addr *=.*|http_listen_addr = \"$http_listen_addr\"|g" \
    -e "s|^https_listen_addr *=.*|https_listen_addr = \"$https_listen_addr\"|g" \
    -e "s|^http_trap_paths *=.*|http_trap_paths = $http_trap_paths_literal|g" \
    "$out_file"
}
...
# -------------------------
# T13 — Honeypot HTTP/HTTPS
# -------------------------
info "T13: probando honeypot HTTP..."

T13_TMP_CFG="$(mktemp /tmp/dm_t13_XXXX.toml)"
T13_LOG_FILE="$(mktemp /tmp/dm_t13_XXXX.log)"
...
info "T13: levantando sensor_agent con honeypot HTTP..."
if [ "$HAVE_STDBUF" = "1" ]; then
  stdbuf -oL -eL ./target/debug/sensor_agent --config "$T13_TMP_CFG" >"$T13_LOG_FILE" 2>&1 &
else
  ./target/debug/sensor_agent --config "$T13_TMP_CFG" >"$T13_LOG_FILE" 2>&1 &
fi
T13_AGENT_PID=$!

wait_for_log "http honeypot listening" "$T13_LOG_FILE" 20 || {
  cat "$T13_LOG_FILE" >&2
  die "T13: el honeypot HTTP no quedó escuchando"
}

ok "T13 bootstrap OK (http honeypot listening)"

info "T13: tocando rutas trampa..."
curl -sS -o /dev/null -A "$T13_HTTP_USER_AGENT" \
  "http://127.0.0.1:$T13_HTTP_PORT/admin"

curl -sS -o /dev/null -A "$T13_HTTP_USER_AGENT" \
  "http://127.0.0.1:$T13_HTTP_PORT/wp-login.php"
...
for _ in $(seq 1 10); do
  if grep -qi "http trap request captured" "$T13_LOG_FILE" \
    && grep -qi "/login" "$T13_LOG_FILE" \
    && grep -qi "/admin" "$T13_LOG_FILE" \
    && grep -qi "/wp-login.php" "$T13_LOG_FILE"; then
    break
  fi
  sleep 1
done

grep -qi "http trap request captured" "$T13_LOG_FILE" || {
  cat "$T13_LOG_FILE" >&2
  die "T13: no se registró evento HTTP"
}

grep -qi "/login" "$T13_LOG_FILE" || {
  cat "$T13_LOG_FILE" >&2
  die "T13: no se capturó path /login"
}

grep -qi "/admin" "$T13_LOG_FILE" || {
  cat "$T13_LOG_FILE" >&2
  die "T13: no se capturó path /admin"
}

grep -qi "/wp-login.php" "$T13_LOG_FILE" || {
  cat "$T13_LOG_FILE" >&2
  die "T13: no se capturó path /wp-login.php"
}

grep -qi "$T13_HTTP_USER_AGENT" "$T13_LOG_FILE" || {
  cat "$T13_LOG_FILE" >&2
  die "T13: no se capturó el user-agent"
}

grep -qi "127.0.0.1" "$T13_LOG_FILE" || {
  cat "$T13_LOG_FILE" >&2
  die "T13: no se capturó la IP"
}

grep -qi "POST" "$T13_LOG_FILE" || {
  cat "$T13_LOG_FILE" >&2
  die "T13: no se capturó el método HTTP"
}
...
rm -f "$T13_TMP_CFG" "$T13_LOG_FILE"
unset T13_TMP_CFG
unset T13_LOG_FILE

ok "T13 OK (/login, /admin, /wp-login.php generan evento con método, path, user-agent e IP)"

echo
ok "E2E hasta T13 COMPLETADO (T10 en dev es informativo)."